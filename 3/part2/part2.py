...
def main():
    ...
    try:
        sqliteConnection = sqlite3.connect('day3-p2.db')
        cursor = sqliteConnection.cursor()

        for table in ('cable0', 'cable1'):
            cursor.execute(f'CREATE TABLE IF NOT EXISTS {table} (step INTEGER PRIMARY KEY AUTOINCREMENT, cable_type INTEGER NOT NULL, x_coordinate INTEGER NOT NULL, y_coordinate INTEGER NOT NULL);')
        ...
        cursor.execute("""SELECT min(cable0.step + cable1.step)
